...
def pokemon():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		#Break if broken
		if not response.ok:
			return render_template('pokemon.html', error='Error: Bad request')
		#Break if broken (again)
		try:
			info = get_info(response.json())
			return render_template('pokemon.html', info=info)
		except:
			return render_template('pokemon.html', error="Error: Error processing response")
